...
export default function UploadPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) setFormData({ ...formData, file, filePreview: URL.createObjectURL(file) });
  };
      ...
      <div className="p-4 space-y-6">
        {!uploadType ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="border-2 border-dashed border-muted rounded-2xl p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mb-6">{isArtist ? 'Share your music, videos, or photos' : 'Share videos, photos, or stories'}</p>
            ...
            {!isArtist && (
              <div className="mt-6 p-4 rounded-xl bg-muted/50 text-left">
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  <button onClick={() => navigate('/settings')} className="text-primary hover:underline ml-1">Apply for artist status</button>
                </p>
              </div>
            )}
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
            ...
            <div className="border-2 border-dashed border-muted rounded-2xl p-6 text-center">
              {formData.filePreview ? (
                ...
                <label className="cursor-pointer block">
                  ...
                  <input type="file" accept={uploadType === 'audio' ? 'audio/*' : uploadType === 'video' ? 'video/*' : 'image/*'} className="hidden" onChange={handleFileChange} />
                </label>
              )}
            </div>
            ...
            {uploadType !== 'story' && (
              <div className="flex items-center justify-between p-4 rounded-xl bg-muted/50">
                ...
                <Switch checked={formData.addToStory} onCheckedChange={(checked) => setFormData({ ...formData, addToStory: checked })} />
              </div>
            )}
          </motion.div>
        )}
